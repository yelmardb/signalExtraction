...
void PileUpCorrection::InitHistograms() {
    ...
    _hnData = (THnSparseD*)extractHistogram(Form("%s:/pcm-qc-mc/V0/primary/hDCAz_Pt_collType",_stDataFileName));
    if (!_hnData) {
    ...
    _hnMC = (THnSparseD*)extractHistogram(Form("%s:/pcm-qc-mc/V0/primary/hDCAz_Pt_collType",_stMCFileName));
    if (!_hnMC) {
        cerr << "ERROR: could not find histogram\n";
        return;
    }
...
//        ROOT::Math::MinimizerOptions::SetDefaultMinimizer(“Minuit2”);
        ...
        _FractionFitter->Constrain(1, 0.5, 1.0); // secDecay or sec

        auto _Fitter = _FractionFitter->GetFitter();

        _Fitter->Config().ParSettings(0).Set("valCol", _mcValColIntegral / _mcTotalIntegral, _StepSize,0.5, 1.0);
        _Fitter->Config().ParSettings(1).Set("notValCol", _mcNotValColIntegral / _mcTotalIntegral, _StepSize, 0.5, 1.0);
        ...
        if (0 == _FitStatus) { // fit successful
            ...
            cout << "\nnew fractions: \n";
            cout << "\t validated col./total: " << factorValCollisions << " +- " << factorValCollisionsError
            << "\n";
            cout << "\t not validated col./total: " << factorNotValCollisions << " +- "
            << factorSecondayError << "\n";

            cout << "\nResulting scaling factors: \n";
            cout << "\t validated col./total: " << factorValCollisions / (_mcValColIntegral / _mcTotalIntegral) << "\n";
            _ScalingSecondary = factorNotValCollisions / (_mcNotValColIntegral/_mcTotalIntegral);

            cout << "\t scaling pile up: " << _ScalingSecondary << "\n";
...
void PileUpCorrection::MakePlot(){
    ...
    SetHistogramPropertiesAlt(_h1DataDCA, "DCA_{xy}", "counts", 1);

    TH1D* _result = (TH1D*)_FractionFitter->GetPlot();
    _result->SetName("_result");
    SetHistogramPropertiesAlt(_result, "DCA_{xy}", "counts", 2);
    SetHistogramPropertiesAlt(_ValColFit, "DCA_{xy}", "counts", 3);
    SetHistogramPropertiesAlt(_NotValColFit, "DCA_{xy}", "counts", 4);

    TLegend* _leg = new TLegend(0.7, 0.7, 0.9, 0.9); 
    _leg->AddEntry((TObject*)0x0, Form("%3.2lf < #it{p}_{T} < %3.2lf", _pTMin, _pTMax), "");
    _leg->AddEntry(_h1DataDCA, "data");
    _leg->AddEntry(_result, "template prediction");
    _leg->AddEntry(_ValColFit, "val. collisions");
    _leg->AddEntry(_NotValColFit, "unval. collisions");
    ...
    histArray->Add(_h1DataDCA);
    histArray->Add(_result);
    histArray->Add(_ValColFit);
    histArray->Add(_NotValColFit);
    histArray->Add(_leg);

    TObjArray* ratioArray = new TObjArray();
    ...
    SetHistogramPropertiesAlt(_ratio, "DCA_{xy}", "Data/template", 1);
    _ratio->GetXaxis()->SetRangeUser(_PlotRangeMin, _PlotRangeMax);
    _ratio->GetYaxis()->SetRangeUser(0.5, 1.5);
    ratioArray->Add(_ratio);

    TCanvas* c1 = makeCanvas(histArray, ratioArray,"CMYK|NoTime LogY quiet", 0, 0);



    // DefaultPlot plotPulls;
    // SetHistogramPropertiesAlt(_hPulls, "Pulls", "norm. counts", 6);
