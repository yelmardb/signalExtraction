...
#ifndef SecondarySacling_hpp
#define SecondarySacling_hpp
...
// todo
// do always 1 and 2 template fit
// add bin-shift functionality to match peak positions
// figure out what leads fits to arbitrarily fail (understand how to do it
// right) find optimal pt binning

class SecondaryScaling {
  THnSparseD*         _hnData = nullptr;//DCAxy:pT:nTracks:MCinfo
  THnSparseD*         _hnMC = nullptr;//DCAxy:pT:nTracks:MCinfo; MCinfo --> pri:sec:decay:material
  ...
  TH1D*               _h1PriDCA = nullptr;
  TH1D*               _h1SecDCA = nullptr;
  TH1D*               _h1SecDecayDCA = nullptr;
  TH1D*               _h1SecMaterialDCA = nullptr;
  ...
  std::vector<double> _multBinning{};
  double              _dataIntegral = 0;
  double              _mcPriIntegral = 0;
  double              _mcSecIntegral = 0;
  double              _mcSecDecayIntegral = 0;
  double              _mcSecMaterialIntegral = 0;
  ...
  float               _ScalingDecay = 1;
  float               _ScalingMaterial = 1;
  float               _pVal = 0.05;
  int                 _FitStatus = -1;
  int                 _RebinFactor = 31;
  ...
  void SetMultMin(float mult){_MultMin=mult;}
  void SetMultMax(float mult){_MultMax=mult;}
  void SetMultRange(float multMin, float multMax) {_MultMin=multMin; _MultMax=multMax;}
  void SetCenttMin(float cent){_CentMin=cent;}
  void SetCentMax(float cent){_CentMax=cent;}
  void SetCentRange(float centMin, float centMax) {_CentMin=centMin; _CentMax=centMax;}
  ...
  void SetMultBinning(std::vector<double> bins) {_multBinning=bins;}
 ...
 // auto const GetMultBinning() const {return _multBinning;}
